# -------------------------------------------------------------------------
# Erstellung des Gesetzesentwurfs in der Fassung der Beschlussempfehlung
# -------------------------------------------------------------------------
library(stringr)

# Erst den Beschlussempfehlungstext generieren. 
# Dann Änderungsanträge darauf anwenden sofern explicitref nicht auf etwas anderes verweist.

legistext$version_committee <- NA
committee$action_matrix <- NA
committee$manual <- 0

# ------------------------------------------------------------
# Szenario: Gesetzesentwurf bleibt im Ausschuss unverändert
# ------------------------------------------------------------
aenderungs_identifier <- c("Änderung", "Fassung", "gefasst", "geändert")

ausschuss_geandert <- which(str_detect(committee$rawtext_left, aenderungs_identifier[1]) == T | 
                            str_detect(committee$rawtext_left, aenderungs_identifier[2]) == T | 
                            str_detect(committee$rawtext_left, aenderungs_identifier[3]) == T | 
                            str_detect(committee$rawtext_left, aenderungs_identifier[4]) == T ) 

ausschuss_nicht_geandert <- (1:nrow(committee))[-ausschuss_geandert]
legis_nicht_geandert <- which(is.element(legistext$idDrucksache, committee$idDrucksacheLegis[ausschuss_nicht_geandert]))
legistext$version_committee[legis_nicht_geandert] <- legistext$bill_list[legis_nicht_geandert]
...
# --------------------------------------------------------------------------------------------------------------------------------
# Szenario: Beschlussempfehlung beschließt nicht Änderungen am ursprünglichen Gesetzesentwurf, sondern eine genuin neue Fassung
# Dann einfach die neue Fassung als version_committee nehmen
# --------------------------------------------------------------------------------------------------------------------------------
# Loop über alle entsprechenden amendment_lists, die im Ausschuss verändert werden
for (committee_row in 1: nrow(committee)) {

tryCatch({
# amendment_list <- committee$amendment_list[committee_row][[1]][[1]]

# Loop über alle entsprechenden amendment_lists, die im Ausschuss verändert werden
if (is.null(names(committee$amendment_list[committee_row][[1]][[1]][[1]][[1]])) == F)
  amendment_list <- committee$amendment_list[committee_row][[1]][[1]][[1]][[1]]
if (is.null(names(committee$amendment_list[committee_row][[1]][[1]][[1]])) == F)
  amendment_list <- committee$amendment_list[committee_row][[1]][[1]][[1]]
if (is.null(names(committee$amendment_list[committee_row][[1]][[1]])) == F)
  amendment_list <- committee$amendment_list[committee_row][[1]][[1]]
if (is.null(names(committee$amendment_list[committee_row][[1]])) == F)
  amendment_list <- committee$amendment_list[committee_row][[1]]

if (is.null(names(committee$amendment_list[committee_row][[1]][[1]][[1]][[1]])) == T &
    is.null(names(committee$amendment_list[committee_row][[1]][[1]][[1]])) == T &
    is.null(names(committee$amendment_list[committee_row][[1]][[1]])) == T &
    is.null(names(committee$amendment_list[committee_row][[1]])) == T) 
  amendment_list <- committee$amendment_list[committee_row][[1]]
  
  
  
  
# Ist der usprüngliche Entwurf überhaupt verändert worden?
if (is.element(committee_row, ausschuss_nicht_geandert)==T) {
  committee$action_matrix[committee_row] <- "none"
  next
}


if (is.list(amendment_list) == F){ amendment_list <- list(amendment_list)}
legistext_row <- which(legistext$idDrucksache==committee$idDrucksacheLegis[committee_row])

if (str_detect(paste(amendment_list[[1]], collapse=" "), "\\b(folgende|folgender) Fassung\\b") == T){
  legistext$version_committee[legistext_row] <- list(amendment_list)
  committee$action_matrix[committee_row] <- "none"
  next
}
...
if (length(which(sums>1)) > 0) 
  committee$manual[committee_row] <- 1
...
for (i in 1: length(list.dims)){
  ...
  for (i in 1: length(list.dims)){
    ...
    if (length(na.omit(str_extract(action_text[i], action_words))) == 1)
      action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.fkt"] <- na.omit(str_extract(action_text[i], action_words))
...
if (is.element(committee$idDrucksacheLegis[committee_row], neu_gesetze) == T) {
    ...
    if (nrow(action_matrix)>1){
      ...
      for (i in 2:nrow(action_matrix)){
        ...
        if (str_detect(list.dims[i], "fehlt") == F) {
          if (str_detect(list.dims[i-1], "fehlt") == T)
            abschnitt_uebertrag <- names(which(is.na(action_matrix[i-1,c("ref.art", "ref.num", "ref.buch", "ref.buchbuch", "ref.para", "ref.absatz", "ref.satz")])==F))
          
          uebertrag <- names(which(is.na(action_matrix[i, abschnitt_uebertrag])))
          action_matrix[i,uebertrag] <- action_matrix[i-1,uebertrag]
        }
      }  
...
if (is.element(committee$idDrucksacheLegis[committee_row], aenderungs_gesetze) == T) {
...
for (i in 1:length(list.dims)){
  ...
  if (length(action_matrix[action_matrix[,"vorschlag.idx"]==i,"vorschlag.idx"]) == 1) {
    ...
    # ---------------------------
      ...
      # Referenz: "Satz 1"
      if (str_count(action_text[i], "\\bSatz\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz"] <- str_extract(action_text[i], "\\bSatz\\s[:digit:]+\\b")
      
      # Referenz: "Satz 1 und Satz 2"
      if (str_count(action_text[i], "\\bSatz\\s[:digit:]\\sund\\bSatz\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz"] <- str_extract(action_text[i], "\\bSatz\\s[:digit:]+\\sund\\bSatz\\s[:digit:]+\\b")
      
      
      # Referenz: "Satz 1 und 2"
      if (str_count(action_text[i], "\\bSatz\\s[:digit:]\\sund\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz"] <- str_extract(action_text[i], "\\bSatz\\s[:digit:]+\\sund\\s[:digit:]+\\b")
      ...
      if (str_count(action_text[i], "\\bSatz\\s[:digit:],\\s[:digit:]\\sund\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz"] <- str_extract(action_text[i], "\\bSatz\\s[:digit:]+,\\s[:digit:]\\sund\\s[:digit:]+\\b")
      ...
      if (str_count(action_text[i], "\\bSatz\\s[:digit:]\\sbis\\bSatz\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz"] <- str_extract(action_text[i], "\\bSatz\\s[:digit:]+\\sbis\\bSatz\\s[:digit:]+\\b")
      
      # Referenz: "Satz 1 bis 3"
      if (str_count(action_text[i], "\\bSatz\\s[:digit:]\\sbis\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz"] <- str_extract(action_text[i], "\\bSatz\\s[:digit:]+\\sbis\\s[:digit:]+\\b")
      ...
      # Referenz: "Satz 1"
      if (str_count(neunum_text[1], "\\bSatz\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz"] <- str_extract(neunum_text[1], "\\bSatz\\s[:digit:]+\\b")
      
      # Referenz: "Satz 1 und Satz 2"
      if (str_count(neunum_text[1], "\\bSatz\\s[:digit:]\\sund\\bSatz\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz"] <- str_extract(neunum_text[1], "\\bSatz\\s[:digit:]+\\sund\\bSatz\\s[:digit:]+\\b")
      
      
      # Referenz: "Satz 1 und 2"
      if (str_count(neunum_text[1], "\\bSatz\\s[:digit:]\\sund\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz"] <- str_extract(neunum_text[1], "\\bSatz\\s[:digit:]+\\sund\\s[:digit:]+\\b")
      ...
      # Referenz: "Satz 1, 2 und 3"
      if (str_count(neunum_text[1], "\\bSatz\\s[:digit:],\\s[:digit:]\\sund\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz"] <- str_extract(neunum_text[1], "\\bSatz\\s[:digit:]+,\\s[:digit:]\\sund\\s[:digit:]+\\b")
      ...
      # Referenz: "Satz 1 bis Satz 3"
      if (str_count(neunum_text[1], "\\bSatz\\s[:digit:]\\sbis\\bSatz\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz"] <- str_extract(neunum_text[1], "\\bSatz\\s[:digit:]+\\sbis\\bSatz\\s[:digit:]+\\b")
      
      # Referenz: "Satz 1 bis 3"
      if (str_count(neunum_text[1], "\\bSatz\\s[:digit:]\\sbis\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz"] <- str_extract(neunum_text[1], "\\bSatz\\s[:digit:]+\\sbis\\s[:digit:]+\\b")
      ...
      # Referenz: "Satz 1"
      if (str_count(neunum_text[2], "\\bSatz\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz.neu"] <- str_extract(neunum_text[2], "\\bSatz\\s[:digit:]+\\b")
      
      # Referenz: "Satz 1 und Satz 2"
      if (str_count(neunum_text[2], "\\bSatz\\s[:digit:]\\sund\\bSatz\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz.neu"] <- str_extract(neunum_text[2], "\\bSatz\\s[:digit:]+\\sund\\bSatz\\s[:digit:]+\\b")
      
      
      # Referenz: "Satz 1 und 2"
      if (str_count(neunum_text[2], "\\bSatz\\s[:digit:]\\sund\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz.neu"] <- str_extract(neunum_text[2], "\\bSatz\\s[:digit:]+\\sund\\s[:digit:]+\\b")
      ...
      # Referenz: "Satz 1, 2 und 3"
      if (str_count(neunum_text[2], "\\bSatz\\s[:digit:],\\s[:digit:]\\sund\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz.neu"] <- str_extract(neunum_text[2], "\\bSatz\\s[:digit:]+,\\s[:digit:]\\sund\\s[:digit:]+\\b")
      ...
      # Referenz: "Satz 1 bis Satz 3"
      if (str_count(neunum_text[2], "\\bSatz\\s[:digit:]\\sbis\\bSatz\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz.neu"] <- str_extract(neunum_text[2], "\\bSatz\\s[:digit:]+\\sbis\\bSatz\\s[:digit:]+\\b")
      
      # Referenz: "Satz 1 bis 3"
      if (str_count(neunum_text[2], "\\bSatz\\s[:digit:]\\sbis\\s[:digit:]+\\b") == 1) 
        action_matrix[action_matrix[,"vorschlag.idx"]==i,"ref.satz.neu"] <- str_extract(neunum_text[2], "\\bSatz\\s[:digit:]+\\sbis\\s[:digit:]+\\b")
...
for (i in 1:length(list.dims)){
  ...
  if (length(action_matrix[action_matrix[,"vorschlag.idx"]==i,"vorschlag.idx"]) == 1) {
      ...
      # Referenz: "Wort"
      if (sum(str_count(eval(parse(text=str_c("amendment_list", list.dims[i]))), "(\\bWort\\s([:lower:]|„)\\b|\\b(Wörter|Wörtern)\\s([:lower:]|„)\\b|\\b(Worte|Worten)\\s([:lower:]|„)\\b|\\b(Angabe|Angaben)\\s([:lower:]|„)\\b)")) > 0) 
...
committee$action_matrix[committee_row] <- list(action_matrix)
...
if (nrow(action_matrix) == 0)
  ...
  committee$action_matrix[committee_row] <<- NA
...
# Manuell abweichende Fälle aussortieren
committee$action_matrix[which(committee$idDrucksache=="16/2838")] <- NA
committee$action_matrix[which(committee$idDrucksache=="14/1872")] <- NA
