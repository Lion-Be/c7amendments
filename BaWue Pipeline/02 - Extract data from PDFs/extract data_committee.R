...
# --------------------
  ...
  if (z == 1){
    ...
    # Create empty matrix
    committeepdf <- as.data.frame(matrix(NA, ncol=6, nrow=1))
    colnames(committeepdf) <- c("pdfdocu", "idVorgang", "idDrucksache", "datestemp", "period", "error")
    
    # Fill with information
    committeepdf$idDrucksache <- idDrucksache1
    committeepdf$datestemp <- datestemp1
    committeepdf$period <- period1  
    committeepdf$error <- 0
    ...
    amendmentpropc <- as.data.frame(cbind(amendmentprop, rawtext_left2, rawtext_white2))
    ...
    # Enlarge dataframe
    newdata <- as.data.frame(matrix(NA, ncol= ncol(committeepdf), nrow=1))
    colnames(newdata) <- colnames(committeepdf)  
    ...
    committeepdf <- as.data.frame(rbind(committeepdf, newdata))   
...
committeepdf_err <- as.data.frame(matrix(NA, ncol=ncol(committeepdf), nrow=length(error)))
colnames(committeepdf_err) <- colnames(committeepdf)
committeepdf_err$idDrucksache <- error
committeepdf_err$error <- 1
...
# Merge dataframes
committeepdf <- as.data.frame(rbind(committeepdf, committeepdf_err))
committee <- as.data.frame(rbind(committee, committee_err))
amendmentpropc <- as.data.frame(rbind(amendmentpropc, amendment_err))

#rm(list=setdiff(ls(), c("committeepdf", "committee", "amendmentprop", "pdfs")))



# ---------------------------
# Merge out_list information 
# ---------------------------
  ...
  for (i in 1: nrow(amendmentpropc)){
    # Delete spaces in idDrucksache
    amendmentpropc$idDrucksache[i] <- gsub(" ","",amendmentpropc$idDrucksache[i])
    
    # Match entry in idDrucksache with respective vorgangs_id in out_list
    out_element <- grep(paste("\\b", eval(amendmentpropc$idDrucksache[i]), "\\b", sep=""), fundstellen)  
    
    if (length(out_element) == 1) {
      amendmentpropc$idVorgang[i] <- out_list[[out_element]]["vorgangs_id"][[1]][1]
    }
    
    if (length(out_element) == 2) { 
      amendmentpropc$idVorgang[i] <- list(c(out_list[[out_element[1]]]["vorgangs_id"][[1]][1], out_list[[out_element[2]]]["vorgangs_id"][[1]][1]))
    }
    
    if (length(out_element) == 3) { 
      amendmentpropc$idVorgang[i] <- list(c(out_list[[out_element[1]]]["vorgangs_id"][[1]][1], out_list[[out_element[2]]]["vorgangs_id"][[1]][1], out_list[[out_element[3]]]["vorgangs_id"][[1]][1]))
    }
  ...
  for (i in 1: nrow(committee)){
    
    # Delete spaces in idDrucksache
    committee$idDrucksache[i] <- gsub(" ","",committee$idDrucksache[i])
    committeepdf$idDrucksache[i] <- gsub(" ","",committeepdf$idDrucksache[i])
    
    # committee: Match entry in idDrucksache with respective vorgangs_id in out_list
    out_element <- grep(paste("\\b", eval(committee$idDrucksache[i]), "\\b", sep=""), fundstellen) 
    
    if (length(out_element) > 0) {
      
      if (length(out_element) == 1) {
        committee$idVorgang[i] <- out_list[[out_element]]["vorgangs_id"][[1]][1]
      }
      
      if (length(out_element) == 2) { 
        committee$idVorgang[i] <- list(c(out_list[[out_element[1]]]["vorgangs_id"][[1]][1], out_list[[out_element[2]]]["vorgangs_id"][[1]][1]))
      }
      
      if (length(out_element) == 3) { 
        committee$idVorgang[i] <- list(c(out_list[[out_element[1]]]["vorgangs_id"][[1]][1], out_list[[out_element[2]]]["vorgangs_id"][[1]][1], out_list[[out_element[3]]]["vorgangs_id"][[1]][1]))
      }
      
      #idCommittee
      out_element <- grep(paste("\\b", eval(committee$idDrucksache[i]), "\\b", sep=""), out_list)
      ...
      if (length(out_element) > 1) {
        ...
        element_row <- grep(paste("\\b", eval(committee$idDrucksache[i]), "\\b", sep=""), out_list[[out_element]]["fundstellen_name"][[1]][])
        idCommittee <- out_list[[out_element]]["fundstellen_name"][[1]][element_row]
        idCommittee <- sub('.*Beschlussempfehlung und Bericht ', '', idCommittee)
        idCommittee <- str_extract(idCommittee, "[^[0-9]]+")
        committee$idCommittee[i] <- trimws(idCommittee, which = "right")
      
      
      # committeepdf: Match entry in idDrucksache with respective vorgangs_id in out_list
      out_element <- grep(paste("\\b", eval(committeepdf$idDrucksache[i]), "\\b", sep=""), fundstellen) 
      
      if (length(out_element) == 1) {
        committeepdf$idVorgang[i] <- out_list[[out_element]]["vorgangs_id"][[1]][1]
      }
      
      if (length(out_element) == 2) { 
        committeepdf$idVorgang[i] <- list(c(out_list[[out_element[1]]]["vorgangs_id"][[1]][1], out_list[[out_element[2]]]["vorgangs_id"][[1]][1]))
      }
      
      if (length(out_element) == 3) { 
        committeepdf$idVorgang[i] <- list(c(out_list[[out_element[1]]]["vorgangs_id"][[1]][1], out_list[[out_element[2]]]["vorgangs_id"][[1]][1], out_list[[out_element[3]]]["vorgangs_id"][[1]][1]))
      }
    }
  ...
  fundstellen <- rep(NA, length(out_list))
  for (k in 1: length(out_list)){
    fundstellen[k] <- paste(out_list[[k]]["fundstellen_name"][1:length(out_list[[1]]["fundstellen_name"][[1]])], collapse=" ")
  }
...
# -----------------------------------------------
# Intervention stemming from plausibility check
# -----------------------------------------------

# committee
for (i in 1: nrow(committee) ) {
  
  if (lengths(committee$idVorgang[i]) > 1) {
    committee[i,4 : length(committee) ] <- NA
    committee$error[i] <- 1
  }  
}
...
for (i in 1: nrow(amendmentpropc) ) {
  ...
  if (lengths(amendmentpropc$idVorgang[i]) > 1) {
    amendmentpropc[i,4 : length(amendmentpropc) ] <- NA
    amendmentpropc$error[i] <- 1
  }
}
...
# amendmentpropc$rawtext_left  
for (k in 1: nrow(amendmentpropc)) {
  
  # if there is text in cell
  if (is.na(amendmentpropc$rawtext_left[k]) == FALSE) {
  
  for (i in 1: lengths(amendmentpropc$rawtext_left[k])) {
    
    # if row ends with "-"
    if((str_sub(amendmentpropc$rawtext_left[[k]][i], -1, -1) == "-") == TRUE) {
      
      
      # if first character of next row is lower case
      is.lower <- "[a-z]"
      lowercase <- grepl(pattern = is.lower, x = str_sub(amendmentpropc$rawtext_left[[k]][i+1], 1, 1))
      
      if (lowercase == TRUE) {
        
        # delete "-"  
        amendmentpropc$rawtext_left[[k]][i] <- str_sub(amendmentpropc$rawtext_left[[k]][i], 1, -2)
        
        # clip last word of row to next row
        amendmentpropc$rawtext_left[[k]][i+1] <- paste(c(word(amendmentpropc$rawtext_left[[k]][i], -1), amendmentpropc$rawtext_left[[k]][i+1]), collapse="")
        
        # delete last word of row 
        
        # count number of words
        n.words <- sapply(strsplit(amendmentpropc$rawtext_left[[k]][i], " "), length)
        
        # only keep first to second last word
        amendmentpropc$rawtext_left[[k]][i] <- paste(word(amendmentpropc$rawtext_left[[k]][i], 1:(n.words-1)), collapse=" ")
        ...
        # clip "-" to next row  
        amendmentpropc$rawtext_left[[k]][i+1] <- paste(c(str_sub(amendmentpropc$rawtext_left[[k]][i], -1, -1), amendmentpropc$rawtext_left[[k]][i+1]), collapse="")
...
# committee$rawtext_left
for (k in 1: nrow(committee)) {
  
  # if there is text in cell
  if (is.na(committee$rawtext_left[k]) == FALSE) {
    
  for (i in 1: lengths(committee$rawtext_left[k])) {
    
    # if row ends with "-"
    if((str_sub(committee$rawtext_left[[k]][i], -1, -1) == "-") == TRUE) {
      
      
      # if first character of next row is lower case
      is.lower <- "[a-z]"
      lowercase <- grepl(pattern = is.lower, x = str_sub(committee$rawtext_left[[k]][i+1], 1, 1))
      
      if (lowercase == TRUE) {
        
        # delete "-"  
        committee$rawtext_left[[k]][i] <- str_sub(committee$rawtext_left[[k]][i], 1, -2)
        
        # clip last word of row to next row
        committee$rawtext_left[[k]][i+1] <- paste(c(word(committee$rawtext_left[[k]][i], -1), committee$rawtext_left[[k]][i+1]), collapse="")
        
        # delete last word of row 
        
        # count number of words
        n.words <- sapply(strsplit(committee$rawtext_left[[k]][i], " "), length)
        
        # only keep first to second last word
        committee$rawtext_left[[k]][i] <- paste(word(committee$rawtext_left[[k]][i], 1:(n.words-1)), collapse=" ")
        ...
        # clip "-" to next row  
        committee$rawtext_left[[k]][i+1] <- paste(c(str_sub(committee$rawtext_left[[k]][i], -1, -1), committee$rawtext_left[[k]][i+1]), collapse="")
    ...
    } # end if((str_sub(committee$rawtext_left[[k]][i], -1, -1) == "-") == TRUE)
...
colnames(amendmentpropc)[24] <- "error_extract"
amendmentpropc$amendment_list <- 0
amendmentpropc$error_list <- 0

colnames(committee)[9] <- "error_extract"
committee$committee_list <- 0
committee$error_list <- 0

# --------------------------------
# Haushaltsgesetzgebung löschen
# --------------------------------

which_haushalt_amendc <- which(is.element(amendmentpropc$idVorgang, vorgang_haushalt), T)
amendmentpropc <- amendmentpropc[-which_haushalt_amendc,]

which_haushalt_amendcom <- which(is.element(committee$idVorgang, vorgang_haushalt), T)
committee <- committee[-which_haushalt_amendcom,]

which_haushalt_amendcom <- c("15/1101", "15/1102", "15/1103", "15/1104", "15/1105", "15/1106", "15/1107", "15/1108", "15/1109", 
                             "15/1110", "15/1111", "15/1112", "15/1113", "15/1114", "15/1115", "15/1116", "15/1117",
                             "15/2601", "15/2602", "15/2603", "15/2604", "15/2605", "15/2606", "15/2607", "15/2608", "15/2609",
                             "15/2610", "15/2611", "15/2612", "15/2613", "15/2614", "15/2615", "15/2616", "15/2617",
                             "15/6000", "15/6001", "15/6002", "15/6003", "15/6004", "15/6005", "15/6006", "15/6007", "15/6008",
                             "15/6009", "15/6010", "15/6011", "15/6012", "15/6013", "15/6014", "15/6015", "15/6016", "15/6017",
                             "16/1301", "16/1302", "16/1303", "16/1304", "16/1305", "16/1306", "16/1307", "16/1308", "16/1309",
                             "16/1310", "16/1311", "16/1312", "16/1313", "16/1314", "16/1315", "16/1316", "16/1317", "16/1318")

which_haushalt_amendcom2 <- which(is.element(committee$idDrucksache, which_haushalt_amendcom), T)
committee <- committee[-which_haushalt_amendcom2,]

which_haushalt_amendc <- which(is.element(amendmentpropc$idDrucksache, which_haushalt_amendcom), T)
amendmentpropc <- amendmentpropc[-which_haushalt_amendc,]


which_haushalt_amendcom <- c("14/801", "14/802", "14/803", "14/804", "14/805", "14/806", "14/807", "14/808", "14/809", "14/810",
                             "14/811", "14/812", "14/813", "14/814", "14/816", "14/3601", "14/3602", "14/3603", "14/3604", "14/3605",
                             "14/3608", "14/3609", "14/3610", "14/3612", "14/3614", "14/5701", "14/5702", "14/5703", "14/5704",
                             "14/5705", "14/5706", "14/5707", "14/5708", "14/5709", "14/5710", "14/5711", "14/5712", "14/5714",
                             "14/5715", "14/5716")
