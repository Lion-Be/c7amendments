#------------------------------------
## Download der Dokumente auf PARLIS (Stand: 30.07.2018)
#------------------------------------
...
button <- driver$findElement(using = "id", value = "a-menu-suchegesetz")
button$clickElement()
Sys.sleep(1)

## "Suche starten"
button <- driver$findElement(using = "id", value = "SearchDisplayTriggerGesetz")
button$clickElement()
Sys.sleep(5)
...
index <- 1
	...
	buttons <- driver$findElements(using = "class", value = "efxZoomTabVorgang")

	for(i in 1:length(buttons)){
		driver$mouseMoveToLocation(webElement = buttons[[i]])
		driver$click()
		Sys.sleep(1)
	...
	write(source[[1]], file = str_c("~/Desktop/parlis_source/source_", index, ".html"))

	index <- index + 1

	## Nächste Seite 
	button <- driver$findElement(using = "xpath", value = "//a/i[@class='fa fa-forward']")
	button$clickElement()
	Sys.sleep(5)
...
for(i in 1:length(documents)){
	if(!dir.exists(str_c("~/Desktop/out_dat/", vorgang))){
		dir.create(str_c("~/Desktop/out_dat/", vorgang))
	}
	download.file(
		documents[i], 
		destfile = str_c(
